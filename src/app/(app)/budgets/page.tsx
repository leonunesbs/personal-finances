...
export default async function BudgetsPage({ searchParams }: BudgetsPageProps) {
  ...
  const [categories, budget, transactions, previousBudget, yearBudgets] = await Promise.all([
    ...
    supabase.from('monthly_budgets').select('*').eq('user_id', user.id).eq('month', startLabel).maybeSingle(),
    supabase
      .from('transactions')
      ...
      .eq('user_id', user.id),
    supabase.from('monthly_budgets').select('id').eq('user_id', user.id).eq('month', previousStartLabel).maybeSingle(),
    supabase
  ...
  const { data: budgetItems } = budget.data?.id
    ? await supabase.from('budget_items').select('*').eq('monthly_budget_id', budget.data.id)
    : { data: [] };
