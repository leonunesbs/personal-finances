...
import { createTransactionSchema, type CreateTransactionFormValues } from "./create-schema";
...
export function useTransactionForm({ accounts, cards }: UseTransactionFormProps) {
  ...
  const isCreditAccountSelected = Boolean(safeSelectedAccountId)
  ...
  const isToAccountCreditSelected = Boolean(safeSelectedToAccountId)
