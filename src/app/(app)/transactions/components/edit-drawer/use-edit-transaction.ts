...
import { editTransactionSchema } from './edit-schema';
import { updateTransaction } from '../../actions';

import type { EditTransactionFormValues } from './edit-schema';
...
export function useEditTransaction({ accounts, cards, transactions }: UseEditTransactionProps) {
  ...
  const editIsToAccountCreditSelected =
    Boolean(editToAccountId) && accountTypeMap.get(editToAccountId ?? '') === 'credit';
  ...
  const handleConfirmEditInstallments = (createFutureInstallments: boolean) => {
    ...
    submitEditTransaction(
      editInstallmentConfirm.transactionId,
      editInstallmentConfirm.values,
